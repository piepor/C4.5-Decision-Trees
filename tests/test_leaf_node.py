...
from attributes import LeafNodeAttributes, AttributeType, DecisionNodeAttributes
from leaf_classes import LeafClasses

@pytest.fixture
def root_node():
    root_attributes = DecisionNodeAttributes(0, "root", "A", AttributeType.CONTINUOUS, 10.0)
    return DecisionNodeContinuous(root_attributes, None)

@pytest.fixture
def leaf_attributes():
    classes = LeafClasses({"target_a": 20, "target_b": 3})
    return LeafNodeAttributes(1, "A <= 10.0", classes)
